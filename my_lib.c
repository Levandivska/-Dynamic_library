...
int my_str_read_file_delim(my_str_t* str, FILE* file, char delimiter){
    ...
    if (!file)
        return -1;
    while (*(str->data+str->size_m-1) != delimiter){
        if (str->size_m  == str->capacity_m)
            my_str_reserve(str, str->capacity_m*2);
        fgets(str->data+str->size_m, 2, file);
        if (str->data+str->size_m == NULL)
            return (size_t)-1u;
        str->size_m++;
        if (*(str->data+str->size_m-1) == '\0')
            break;
    }
    str->size_m--;
    return 0;
...
int my_str_reserve(my_str_t* str, size_t buf_size){
    ...
    if (buf_size > str->capacity_m){
        char* new_buffer = malloc(buf_size+1);
        if (new_buffer==NULL)
            return -1;
        for (size_t i = 0; i < str->size_m; i++)
            *(new_buffer + i) = *(str->data+i);
        free(str->data);
        str->data = new_buffer;
...
int my_str_from_cstr(my_str_t* str, const char* cstr){
    ...
    char* buffer = malloc(cstr_size+1);
    if (buffer==NULL)
        return -1;
    str->data = buffer;
    str->capacity_m = cstr_size;
...
int my_str_insert_cstr(my_str_t* str, const char* from, size_t pos){
    ...
    if (pos >= str->size_m)
        return -1;
    if (str->size_m + clenght(from) < str->capacity_m)
        my_str_reserve(str, str->size_m + clenght(from));
    my_str_t copy_str;
    my_str_create(&copy_str, str->size_m - pos + 1);
    my_str_substr(str, &copy_str, pos, str->size_m);
    str->size_m = pos;
...
int my_str_append(my_str_t* str, const my_str_t* from){
    ...
    if(!from || !from->data)
        return  -1;
    while (str->size_m + from->size_m > str->capacity_m)
        my_str_reserve(str, (str->capacity_m+1)*2);
...
int my_str_substr(const my_str_t* str, my_str_t* to, size_t beg, size_t end){
    ...
    while (to->capacity_m < end - beg)
        my_str_reserve(to, (to->capacity_m+1)*2);
...
int my_str_append_cstr(my_str_t* str, const char* from){
    ...
    while (str->size_m + c_size_m > str->capacity_m)
        my_str_reserve(str, (str->capacity_m+1)*2);
...
int test_func_for_str_find_if(char tofind) {
    ...
    int add = my_str_from_cstr(&to_string,"emma");
